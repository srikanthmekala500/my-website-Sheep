...
let tableSorts = {
    healthy: { column: 'sheepId', direction: 'asc' },
    overdue: { column: 'sheepId', direction: 'asc' },
    corentin: { column: 'sheepId', direction: 'asc' },
    treatment: { column: 'sheepId', direction: 'asc' },
    pregnant: { column: 'sheepId', direction: 'asc' }
};
...
const LOW_STOCK_THRESHOLD = 10; // in kg
...
/**
 * Schedules a single, debounced call to applyFiltersAndRender.
 * This prevents multiple, rapid re-renders when several data sources change at once.
 */
function scheduleRender() {
    ...
    }, 50); // A 50ms delay is usually enough to batch updates.
}
 ...
 * @param {string} column - The property to sort by.
 * @param {string} direction - 'asc' or 'desc'.
 * @returns {number} -1, 0, or 1 for sorting.
 */
function universalSorter(a, b, column, direction) {
    let valA = a[column];
    let valB = b[column];
    ...
    if (['buyingPrice', 'weight', 'temperature'].includes(column)) {
        valA = parseFloat(valA) || 0;
        valB = parseFloat(valB) || 0;
    }

    if (['dateRecorded', 'computedLastUpdate', 'computedFollowUpDate'].includes(column)) {
...
function updateTableSortIcons() {
    ...
    for (const tableKey in tableSorts) {
        const tableEl = document.querySelector(`table[data-table-key="${tableKey}"]`);
        if (tableEl) {
            const { column, direction } = tableSorts[tableKey];
            const activeTh = tableEl.querySelector(`th[data-sort="${column}"]`);
            if (activeTh) {
                activeTh.classList.add('active');
                activeTh.dataset.direction = direction;
            }
        }
    }
}
...
function applyFiltersAndRender() {
    ...
    updateScheduleView();
    renderDashboardOverdueList();
    updateFinancialsDashboard();
    updateWeeklyTrackingView();
    renderExpenditureLog();
}

function renderAllRecordTables() {
    // 1. Pre-process records to add computed fields for sorting
    const processedRecords = allRecords.map(record => {
        let computedLastUpdate = null;
        ...
        if (record.treatments) {
            ...
            if (treatments.length > 0) {
                ...
                if (latestTreatment.treatmentDate) computedLastUpdate = latestTreatment.treatmentDate;
                if (latestTreatment.followUpDate) computedFollowUpDate = latestTreatment.followUpDate;
            }
        }
        return { ...record, computedLastUpdate, computedFollowUpDate };
    });
    ...
    processedRecords.forEach(record => {
        ...
        if (status === 'Healthy' || status === 'Recovering') {
            ...
            if (getFollowUpStatus(record) === 'overdue') {
                ...
                if (status === 'Corentin') corentinRecords.push(record); else underTreatmentRecords.push(record);
            }
    ...
    // 3. Sort records based on state
    healthyRecords.sort((a, b) => universalSorter(a, b, tableSorts.healthy.column, tableSorts.healthy.direction));
    overdueRecords.sort((a, b) => universalSorter(a, b, tableSorts.overdue.column, tableSorts.overdue.direction));
    corentinRecords.sort((a, b) => universalSorter(a, b, tableSorts.corentin.column, tableSorts.corentin.direction));
    underTreatmentRecords.sort((a, b) => universalSorter(a, b, tableSorts.treatment.column, tableSorts.treatment.direction));
    pregnantRecords.sort((a, b) => universalSorter(a, b, tableSorts.pregnant.column, tableSorts.pregnant.direction));
...
function populateExpenditureFilters() {
    ...
    const years = [...new Set(masterFeedInventory.map(item => item.purchaseDate ? new Date(item.purchaseDate).getFullYear() : null).filter(Boolean))].sort((a, b) => b - a);
...
function renderExpenditureLog() {
    // Safety guard: Only render this log if its section is currently visible.
    // This prevents it from accidentally overwriting a different table on another page
    // if there's an accidental ID collision in the HTML.
    ...
    const totalValueEl = document.getElementById('feedInventoryTotalValue');
    const monthFilter = document.getElementById('feedMonthFilter');
    const yearFilter = document.getElementById('feedYearFilter');
    const clearFiltersBtn = document.getElementById('clearFeedFiltersBtn');
    if (!tableBody || !totalValueEl || !monthFilter || !yearFilter || !clearFiltersBtn) return; // Safety check
    ...
    filteredInventory.sort((a, b) => new Date(b.purchaseDate) - new Date(a.purchaseDate));

    let totalValue = 0;
    let lowStockCount = 0;
    if (filteredInventory.length === 0) {
        tableBody.innerHTML = `<tr><td colspan="7" class="text-center p-4">No expenditure records found for the selected period.</td></tr>`;
    } else {
        const rowsHtml = filteredInventory.map(item => {
            // If quantity is null or 0, it's a direct cost. The price is the total.
            // Otherwise, it's price * quantity.
            const hasQuantity = item.quantity && parseFloat(item.quantity) > 0;
            const itemQuantity = hasQuantity ? parseFloat(item.quantity) : 1; // Use 1 for calculation if no quantity
            const itemPrice = parseFloat(item.pricePerKg) || 0;
            const itemTotal = hasQuantity ? itemPrice * itemQuantity : itemPrice;
            totalValue += itemTotal;
            
            let rowClass = '';
            let stockWarningHtml = '';
            if (item.category === 'Feed' && hasQuantity && itemQuantity < LOW_STOCK_THRESHOLD) {
                rowClass = 'table-warning-light';
                stockWarningHtml = `<span class="badge bg-warning text-dark ms-2" title="Stock is below ${LOW_STOCK_THRESHOLD} kg"><i class="fas fa-exclamation-triangle me-1"></i>Low Stock</span>`;
                lowStockCount++;
            }
            ...
            return `
                <tr data-id="${item.id}" class="${rowClass}">
                    ...
                    ${stockWarningHtml}</td>
                    <td class="text-center">${formatDate(item.purchaseDate)}</td>
                    <td class="hide-mobile"><span class="badge bg-secondary">${escapeHTML(item.category || 'N/A')}</span></td>
                    <td class="text-end">${hasQuantity ? formatCurrency(itemPrice) : '-'}</td>
                    <td class="text-end">${itemQuantity || '-'}</td>
                    <td class="text-end fw-bold">${formatCurrency(itemTotal)}</td>
                    <td class="text-center">
                        <button class="btn btn-sm btn-outline-primary js-edit-feed-item" data-feed-id="${item.id}" title="Edit"><i class="fas fa-edit"></i></button>
                        <button class="btn btn-sm btn-outline-danger js-delete-feed-item" data-feed-id="${item.id}" data-feed-name="${escapeHTML(item.name)}" title="Delete"><i class="fas fa-trash"></i></button>
                    </td>
                </tr>
            `;
        }).join('');
    ...
    const lowStockCard = document.getElementById('lowStockWarningCard');
    if (lowStockCard) {
        if (lowStockCount > 0) {
...
function renderScheduleRow(record) {
    ...
    // Determine overall row highlight for any overdue item
    const rowClass = (dewormingStatus.isOverdue || vaccinationStatus.isOverdue) ? 'table-danger-light' : ((dewormingStatus.isUpcoming || vaccinationStatus.isUpcoming) ? 'table-warning-light' : '');

    const renderCareCell = (status, notes, lastDate, icon, title) => {
        const medicationName = (notes || '').trim();
        const medicationHtml = medicationName
            ? `<div class="mt-2"><span class="badge bg-primary-subtle text-primary-emphasis rounded-pill">${escapeHTML(medicationName)}</span></div>`
            : '';

        let cardClass = 'schedule-care-card';
        if (status.isOverdue) cardClass += ' is-overdue';
        else if (status.isUpcoming) cardClass += ' is-upcoming';

        return `
            <td class="align-middle">
                <div class="${cardClass}">
                    <div class="d-flex justify-content-between align-items-start">
                        <h6 class="care-title mb-0"><i class="fas ${icon} fa-fw me-2"></i>${title}</h6>
                        ${renderScheduleStatusBadge(status)}
                    </div>
                    ${medicationHtml}
                    <div class="row mt-2 gx-2">
                        <div class="col-6">
                            <div class="care-label">Due Date</div>
                            <div class="care-value">${status.dueDate ? formatDate(status.dueDate.toISOString().split('T')[0]) : 'N/A'}</div>
                        </div>
                        <div class="col-6 text-end">
                            <div class="care-label">Last Given</div>
                            <div class="care-value">${lastDate ? formatDate(lastDate) : 'N/A'}</div>
                        </div>
                    </div>
                </div>
            </td>`;
...
function openBatchLogModal() {
    ...
    toggleBatchDueDateField(document.getElementById('batchTreatmentType').value);
...
/**
 * Renders a summarized list of overdue follow-ups for the main dashboard.
 */
function renderDashboardOverdueList() {
    const listContainer = document.getElementById('dashboardOverdueList');
    if (!listContainer) return;

    const overdueRecords = allRecords.filter(record => {
        const status = record.healthStatus;
        return (status === 'Corentin' || status === 'Under Treatment') && getFollowUpStatus(record) === 'overdue';
    });

    if (overdueRecords.length === 0) {
        listContainer.innerHTML = '<div class="list-group-item text-center text-muted p-3">No overdue follow-ups. Great job!</div>';
        return;
    }

    // Sort by follow-up date (most overdue first) and limit to 5 for the dashboard
    overdueRecords.sort((a, b) => new Date(a.computedFollowUpDate) - new Date(b.computedFollowUpDate));
    const recordsToDisplay = overdueRecords.slice(0, 5);

    const listHtml = recordsToDisplay.map(record => {
        const followUpDate = record.treatments ? Object.values(record.treatments).find(t => t.followUpDate)?.followUpDate : null;
        return `
            <a href="#" class="list-group-item list-group-item-action js-manage-treatment" data-record-id="${record.id}" data-sheep-id="${record.sheepId}">
                <div class="d-flex w-100 justify-content-between">
                    <h6 class="mb-1">${record.sheepId}</h6>
                    <small class="text-danger">${followUpDate ? formatDate(followUpDate) : ''}</small>
                </div>
                <small class="text-muted">${record.healthStatus}</small>
            </a>
        `;
    }).join('');
...
function updateFinancialsDashboard() {
    // This function provides a more standard financial overview.
    // It assumes your HTML has been updated to support this new design.

    // --- Centralized Monthly Data Calculation ---
    const monthlyData = {};

    // Get global filters to apply to cost calculations
    const yearFilter = document.getElementById('globalYearFilter')?.value || 'all';
    const monthFilter = document.getElementById('globalMonthFilter')?.value || 'all';
    ...
    soldRecords.forEach(record => {
        ...
        const buyingPrice = parseFloat(record.buyingPrice) || 0;
        const treatmentCosts = (record.treatments && typeof record.treatments === 'object')
            // Include all logged costs (medical, feed, etc.) in COGS.
            ? Object.values(record.treatments).reduce((sum, t) => {
                // The cost is now automatically calculated for feed, so we just sum it up.
    ...
    // 2. Aggregate "Feed & Other Costs" (Operating Expenses).
    // This is the single source of truth for the "Feed & Other Costs" column in the monthly summary.
    // It sums the "Total Value" of all items from the "Feed Inventory & Cost Management" log.
    let totalGrossExpenditureForPeriod = 0; // This will hold the filtered total for the top card.
    const expenseBreakdown = {}; // For the pie chart

    masterFeedInventory.forEach(item => {
        if (item.purchaseDate && item.category) {
            const itemDate = new Date(item.purchaseDate + 'T00:00:00');
            if (isNaN(itemDate.getTime())) return;
                ...
                // If quantity is null, it's a direct cost where pricePerKg is the total value.
                // Otherwise, it's price * quantity.
                const hasQuantity = item.quantity != null && parseFloat(item.quantity) > 0;
                const price = parseFloat(item.pricePerKg) || 0;
                const quantity = hasQuantity ? parseFloat(item.quantity) : null;
                const itemTotalCost = hasQuantity ? price * quantity : price;
    
                monthlyData[month].opEx += itemTotalCost;
                
                // Check if the item matches the global filter to calculate totals for the top cards and pie chart.
                ...
                if (yearMatch && monthMatch) {
                    totalGrossExpenditureForPeriod += itemTotalCost;
                    if (!expenseBreakdown[item.category]) {
                        expenseBreakdown[item.category] = 0;
                    }
                    expenseBreakdown[item.category] += itemTotalCost;
                }
        }
    });

    const totalFeedAndOtherCosts = totalGrossExpenditureForPeriod;

    // --- Calculations for Top Stat Cards ---
    const revenue = soldRecords.reduce((sum, r) => sum + (parseFloat(r.salePrice) || 0), 0); // NOSONAR
    
    let costOfGoodsSold = 0;
    soldRecords.forEach(record => {
        const buyingPrice = parseFloat(record.buyingPrice) || 0;
        // Ensure COGS for the top card also includes all logged costs (medical and feed).
        const treatmentCostsOnSold = record.treatments ? Object.values(record.treatments).reduce((sum, t) => sum + (parseFloat(t.cost) || 0), 0) : 0;
        costOfGoodsSold += (buyingPrice + treatmentCostsOnSold);
    });

    const grossProfit = revenue - costOfGoodsSold;
    // Net profit is calculated after subtracting the *unallocated* operating expenses.
    // Feed costs assigned to sheep are already in COGS, so they are not subtracted again here.
    const netProfit = grossProfit - totalFeedAndOtherCosts; 
    ...
    const netProfitEl = document.getElementById('financialsNetProfit');
    if (netProfitEl) {
        netProfitEl.textContent = `₹${netProfit.toFixed(2)}`;
        netProfitEl.classList.remove('text-success', 'text-danger', 'text-gray-800');
        if (netProfit < 0) {
            netProfitEl.classList.add('text-danger');
        } else if (netProfit > 0) {
            netProfitEl.classList.add('text-success');
        } else {
            netProfitEl.classList.add('text-gray-800');
        }
    }

    // 3. Calculate derived metrics (gross and net profit) for monthly summary table
    Object.values(monthlyData).forEach(month => {
        month.grossProfit = month.revenue - month.cogs;
        month.netProfit = month.grossProfit - month.opEx;
    });

    // Set colors for profit/loss cards
    const grossProfitEl = document.getElementById('financialsGrossProfit');
    if (grossProfitEl) {
        grossProfitEl.classList.toggle('text-success', grossProfit >= 0);
        grossProfitEl.classList.toggle('text-danger', grossProfit < 0);
    }

    // --- Render Charts and Detailed Table ---
    renderFinancialsChart(monthlyData); // Pass monthlyData to the chart function
    ...
    renderMonthlyFinancialSummaryTable(monthlyData);
}

function renderFinancialsChart(monthlyData) {
    // const monthlyData = {};

    // // Aggregate revenue and COGS by month from sold records
    ...
    if (sortedMonths.length < 1) {
        ...
        if (financialsChart) { financialsChart.destroy(); financialsChart = null; }
        return;
    }
    ...
    const cogsData = sortedMonths.map(m => monthlyData[m].cogs);
    const profitData = sortedMonths.map(m => monthlyData[m].grossProfit);
    const netProfitData = sortedMonths.map(m => monthlyData[m].netProfit);

    const chartData = {
        labels: labels,
        datasets: [
            { label: 'Revenue', data: revenueData, backgroundColor: 'rgba(40, 167, 69, 0.7)', borderColor: 'rgba(0, 0, 0, 1)', type: 'bar', order: 2 },
            { label: 'Cost of Goods Sold', data: cogsData, backgroundColor: 'rgba(220, 53, 69, 0.7)', borderColor: 'rgba(220, 53, 69, 1)', type: 'bar', order: 2 },
            { label: 'Gross Profit', data: profitData, borderColor: 'rgba(13, 110, 253, 1)', backgroundColor: 'rgba(13, 110, 253, 0.2)', type: 'line', order: 1, tension: 0.3, fill: false },
            { label: 'Net Profit', data: netProfitData, borderColor: 'rgba(255, 193, 7, 1)', backgroundColor: 'rgba(255, 193, 7, 0.2)', type: 'line', order: 0, tension: 0.3, fill: false, borderDash: [5, 5] }
        ]
    };
...
function renderExpenseBreakdownChart(expenseCategories) {
...
/**
 * Renders a detailed table of individual sold sheep transactions for the financials page.
 * @param {Array<Object>} records - The array of sold sheep records to display.
 */
function renderFinancialsDetailTable(records) {
    const tableBody = document.getElementById('financialsDetailTableBody');
    if (!tableBody) {
        // If the element doesn't exist, we don't show an error, as the user might not have added it yet.
        // A console log is helpful for the developer.
        console.warn("UI Warning: Element with ID 'financialsDetailTableBody' not found. The detailed sales breakdown table will not be rendered.");
        return;
    }

    if (records.length === 0) {
        tableBody.innerHTML = `<tr><td colspan="7" class="text-center p-4 text-muted">No sales in the selected period.</td></tr>`;
        return;
    }
    ...
    const rowsHtml = sortedRecords.map(record => {
        ...
        let profitClass = 'text-body-secondary';
        let profitSign = '';
        if (profit > 0) { profitClass = 'text-success'; profitSign = '+'; } 
        else if (profit < 0) { profitClass = 'text-danger'; }

        return `
            <tr>
                <td><a href="#" class="fw-bold profile-link" data-sheep-id="${record.id}">${record.sheepId}</a></td>
                <td>${formatDate(record.saleDate)}</td>
                <td class="text-end">₹${salePrice.toFixed(2)}</td>
                <td class="text-end">₹${buyingPrice.toFixed(2)}</td>
                <td class="text-end">₹${treatmentCosts.toFixed(2)}</td>
                <td class="text-end">₹${totalCost.toFixed(2)}</td>
                <td class="text-end fw-bold ${profitClass}">${profitSign}₹${profit.toFixed(2)}</td>
            </tr>
        `;
    }).join('');
...
function renderMonthlyFinancialSummaryTable(monthlyData) {
    ...
    let totalRevenue = 0, totalCogs = 0, totalGrossProfit = 0, totalOpEx = 0, totalNetProfit = 0;
    ...
    const rowsHtml = sortedMonths.map(monthKey => {
        ...
        return `
            <tr>
                ...
                <td class="text-end">₹${data.opEx.toFixed(2)}</td>
    ...
    const footerHtml = `
        <tr class="fw-bold table-dark">
            ...
            <td class="text-end">₹${totalOpEx.toFixed(2)}</td>
...
function handleAddFeedItem(e) {
    ...
    const priceValue = parseFloat(document.getElementById('feedPricePerKg').value);
    const quantityValue = document.getElementById('feedQuantity').value;
    const hasQuantity = quantityValue && !isNaN(parseFloat(quantityValue));

    // If a quantity is provided, the entered price is per unit.
    // If no quantity is provided, the entered price is the total cost.
    const finalPrice = priceValue;
    // Store null if no quantity is given, so we know it's a direct cost
    const finalQuantity = hasQuantity ? parseFloat(quantityValue) : null;
    ...
    if (!newFeedItem.name || !newFeedItem.category || !newFeedItem.purchaseDate || isNaN(newFeedItem.pricePerKg)) {
        return alert('Please fill out Name, Category, Date, and Price/Cost fields.');
    }

    push(ref(db, 'feedInventory'), newFeedItem).then(() => {
        showToast('Expenditure Added', `${newFeedItem.name} was successfully logged.`);
        e.target.reset();
        document.getElementById('feedPurchaseDate').valueAsDate = new Date();
    }).catch(error => {
...
/**
 * Updates the labels and visibility of fields in the "Add Feed/Expenditure" form
 * based on the selected category.
 */
function updateFeedFormUI() {
    const category = document.getElementById('feedCategory').value;
    const quantityContainer = document.getElementById('feedQuantityContainer');
    const quantityLabel = document.getElementById('feedQuantityLabel');
    const quantityInput = document.getElementById('feedQuantity');
    const priceLabel = document.getElementById('feedPriceLabel');

    if (!quantityContainer || !quantityInput || !priceLabel || !quantityLabel) {
        console.error("Could not update feed form UI: one or more required elements are missing.");
        return;
    }
    ...
    switch (category) {
        case 'Feed':
            ...
            break;
        case 'Medical':
            priceLabel.textContent = 'Price per unit (e.g., ml/tablet) (₹)';
            quantityLabel.textContent = 'Quantity (in ml)';
            quantityInput.placeholder = 'e.g., 1000';
            break;
        case 'Travel':
        case 'Utilities':
        case 'Other':
            priceLabel.textContent = 'Total Cost (₹)';
            quantityContainer.style.display = 'none';
            quantityInput.value = ''; // Clear value when hidden
            break;
        default:
            priceLabel.textContent = 'Unit Price / Cost (₹)';
            quantityInput.placeholder = 'e.g., 50';
            break;
    }
}
...
/**
 * Generates a new, unique Sheep ID.
 * It finds the highest existing ID number and increments it.
 * The format is "S" followed by a zero-padded number (e.g., S001, S012, S123).
 * @param {Array<Object>} allRecords - An array of all sheep records.
 * @returns {string} The new unique sheep ID.
 */
function generateNewSheepId(allRecords) {
    const prefix = 'MS';
    let maxId = 0;

    // Find the highest numeric part of existing IDs
    allRecords.forEach(record => {
        if (record.sheepId && record.sheepId.startsWith(prefix)) {
            const numericPart = parseInt(record.sheepId.substring(prefix.length), 10);
...
function handleAddRecord(e) {
    ...
    const getValue = (id, defaultValue = null) => {
        const el = document.getElementById(id);
        if (!el) { console.error(`Critical Error: Form element with ID '${id}' is missing from the HTML.`); return defaultValue; }
        return el.value;
    };

    const newSheepId = generateNewSheepId(masterAllRecords);
    ...
    if (!newRecord.dateRecorded) {
        alert("Date is required. Could not submit form because a required field element is missing from the HTML.");
        return;
    }
...
function handleUpdateRecord(e) {
    ...
    update(ref(db, `sheepHealthRecords/${recordId}`), updatedData).then(() => {
        editSheepModal.hide();
        showToast('Update Successful', `Record for Sheep ID ${updatedData.sheepId} has been updated.`);
    }).catch(error => {
...
function handleUpdateSchedule(e) {
    ...
    const sheepId = document.getElementById('scheduleModalSheepId').textContent;
    update(ref(db, `sheepHealthRecords/${recordId}`), updatedData).then(() => {
        if (editScheduleModal) editScheduleModal.hide();
        showToast('Schedule Updated', `The preventative care schedule for Sheep ID ${sheepId} has been updated.`);
    }).catch(error => {
    ...
    if (confirm('Are you sure you want to mark this sheep as deceased and move it to the archive? This action moves the record and cannot be easily undone.')) {
        ...
        push(ref(db, 'sheepArchivedRecords'), archivedRecord).then(() => {
            remove(ref(db, `sheepHealthRecords/${recordId}`)).then(() => {
                showToast('Record Archived', `Sheep ID ${recordToArchive.sheepId} has been moved to the archive.`, 'info');
            });
        }).catch(error => {
...
function deleteFeedItem(feedId, feedName) {
    if (confirm(`Are you sure you want to delete "${feedName}" from your expenditure log? This action cannot be undone.`)) {
        remove(ref(db, `feedInventory/${feedId}`)).then(() => {
            showToast('Item Deleted', `"${feedName}" was successfully deleted.`, 'danger');
        }).catch(error => {
...
function deleteSoldRecord(recordId, sheepId) {
    if (confirm(`Are you sure you want to PERMANENTLY DELETE the sale record for sheep "${sheepId}"? This action cannot be undone.`)) {
        remove(ref(db, `sheepSaledRecords/${recordId}`)).then(() => {
            showToast('Sale Record Deleted', `The sale record for Sheep ID ${sheepId} has been deleted.`, 'danger');
        }).catch(error => {
...
function deleteArchivedRecord(recordId, sheepId) {
    if (confirm(`Are you sure you want to PERMANENTLY DELETE the archived record for sheep "${sheepId}"? This action cannot be undone.`)) {
        remove(ref(db, `sheepArchivedRecords/${recordId}`)).then(() => {
            showToast('Archived Record Deleted', `The archived record for Sheep ID ${sheepId} has been deleted.`, 'danger');
        }).catch(error => {
            console.error("Error deleting archived record:", error);
            alert("Failed to delete archived record: " + error.message);
        });
    }
}
...
function handleSaleSubmit(e) {
    ...
    push(ref(db, 'sheepSaledRecords'), soldRecord).then(() => {
        remove(ref(db, `sheepHealthRecords/${recordId}`)).then(() => {
            saleSheepModal.hide();
            e.target.reset();
            showToast('Sale Successful', `Sheep ID ${soldRecord.sheepId} has been marked as sold.`, 'success');
        });
    }).catch(error => {
...
function openTreatmentLog(recordId, sheepId, entryIdToEdit = null) {
    ...
    if (masterFeedInventory.length > 0) {
        feedSelect.innerHTML = masterFeedInventory
            .map(f => {
                const unit = f.category === 'Feed' ? 'kg' : (f.category === 'Medical' ? 'ml' : 'units');
                const quantityDisplay = (f.quantity || 0).toFixed(1);
                return `<option value="${f.id}" data-price="${f.pricePerKg}" data-quantity="${f.quantity || 0}" data-category="${f.category}">
                            ${f.name} (${quantityDisplay} ${unit} left)
                        </option>`;
            }).join('');
    } else {
    ...
    // Pre-fill symptoms from the latest treatment if available and not editing
    if (!entryIdToEdit) {
        const record = allRecords.find(r => r.id === recordId);
    ...
    // If an entryId is provided for editing, fetch it and populate the form
    if (entryIdToEdit) {
        const entryRef = ref(db, `sheepHealthRecords/${recordId}/treatments/${entryIdToEdit}`);
        onValue(entryRef, (snapshot) => {
            ...
            toggleTreatmentFormForFeed(treatmentType);

            document.getElementById('treatmentEntryId').value = entryIdToEdit;
...
function deleteTreatmentEntry(recordId, entryId) {
    if (confirm('Delete this treatment entry?')) {
        remove(ref(db, `sheepHealthRecords/${recordId}/treatments/${entryId}`)).then(() => {
            showToast('Treatment Deleted', 'The treatment entry has been successfully removed.', 'danger');
        }).catch(error => {
...
function handleSaveTreatment(e) {
    ...
    // Validate feed quantity before proceeding
    if (treatmentType === 'Feed' && !validateFeedQuantity(true)) {
        // The validate function shows an alert if the second argument is true.
        return; // Stop execution
    }

    // If the treatment is 'Feed', the cost is automatically calculated and read-only.
    // For other types, it's user-entered.
    let finalCost = null;
    if (costEl) {
        const costValue = parseFloat(costEl.value);
        if (!isNaN(costValue)) finalCost = costValue;
    }
    ...
    if (treatmentType === 'Feed' && !isNaN(dosage)) {
        ...
        if (feedItem) {
            const newQuantity = (feedItem.quantity || 0) - dosage;
            allUpdates[`feedInventory/${feedId}/quantity`] = newQuantity;
        }
    }
...
/**
 * Validates the entered feed quantity against the available stock in real-time.
 * @param {boolean} [showAlert=false] - If true, shows a blocking alert on failure. Otherwise, shows inline validation.
 * @returns {boolean} - True if the quantity is valid, false otherwise.
 */
function validateFeedQuantity(showAlert = false) {
    const feedSelect = document.getElementById('treatmentFeedType');
    const quantityInput = document.getElementById('dosage');
    const selectedOption = feedSelect.options[feedSelect.selectedIndex];

    if (!selectedOption) return true; // No feed selected, so no validation needed.
    ...
    // Find the feedback div, or create it if it doesn't exist
    let feedbackDiv = quantityInput.nextElementSibling;
    if (!feedbackDiv || !feedbackDiv.classList.contains('invalid-feedback')) {
        feedbackDiv = document.createElement('div');
        feedbackDiv.className = 'invalid-feedback';
        quantityInput.parentNode.appendChild(feedbackDiv);
    }

    if (!isNaN(requestedQuantity) && requestedQuantity > availableQuantity) {
        const errorMsg = `Quantity cannot exceed available stock (${availableQuantity.toFixed(1)} kg).`;
        if (showAlert) {
            alert(errorMsg);
        }
...
function handleBatchSaveTreatment(e) {
    ...
    const manualDueDate = document.getElementById('batchManualVaccinationDueDate')?.value || null;
    ...
    recordIds.forEach(recordId => {
        ...
        if (treatmentType === 'Deworming') {
            ...
            allUpdates[`sheepHealthRecords/${recordId}/lastVaccinationNotes`] = entryData.treatmentNotes;
            // Set or clear the manual due date
            allUpdates[`sheepHealthRecords/${recordId}/manualVaccinationDueDate`] = manualDueDate;
        }
    });
...
function toggleBatchDueDateField(type) {
    const dueDateContainer = document.getElementById('batchManualDueDateContainer');
    if (dueDateContainer) {
        if (type === 'Vaccination') {
            dueDateContainer.classList.remove('hidden');
        } else {
            dueDateContainer.classList.add('hidden');
            const input = dueDateContainer.querySelector('input');
            if (input) input.value = ''; // Clear value when hiding
        }
    }
}
...
function handleUpdateSoldRecord(e) {
    ...
    update(ref(db, `sheepSaledRecords/${recordId}`), updatedData)
        .then(() => {
            ...
            showToast('Sale Record Updated', `The sale details for Sheep ID ${sheepId} have been updated.`);
        })
        .catch(error => {
...
function openEditFeedModal(feedId) {
    ...
    if (!modalEl) {
        alert('Error: The HTML for the Edit Expenditure modal is missing from index.html.');
        console.error("Could not open edit modal because the element with ID 'editFeedModal' was not found.");
        return;
    }
    ...
    document.getElementById('editFeedCategory').value = item.category || '';
    ...
    document.getElementById('editFeedQuantity').value = item.quantity || '';

    const editFeedModal = bootstrap.Modal.getInstance(modalEl) || new bootstrap.Modal(modalEl);
    editFeedModal.show();
}
...
function handleUpdateFeedItem(e) {
    ...
    if (!feedId) return alert('Error: No item ID found.');

    const quantityValue = document.getElementById('editFeedQuantity').value.trim();
    const quantity = (quantityValue && !isNaN(parseFloat(quantityValue))) ? parseFloat(quantityValue) : null;
    
    const updatedData = {
        name: document.getElementById('editFeedName').value.trim(),
        category: document.getElementById('editFeedCategory').value,
    ...
    if (!updatedData.name || !updatedData.category || !updatedData.purchaseDate || isNaN(updatedData.pricePerKg)) {
    ...
    update(ref(db, `feedInventory/${feedId}`), updatedData).then(() => {
        ...
        if (editFeedModal) editFeedModal.hide();
        showToast('Update Successful', `${updatedData.name} has been updated.`);
    }).catch(error => {
...
function handleSaveSettings(e) {
    ...
    update(ref(db, `users/${user.uid}/settings`), newSettings).then(() => {
        ...
        // Update the local settings object immediately after saving
...
function updateProfileView() {
    ...
    if (currentSelection) {
        // This is the key fix: Re-render the profile if one is selected to reflect data changes.
        renderProfileForSheep(currentSelection);
    } else {
...
function renderProfileForSheep(recordId) {
    ...
    renderProfileTimelineTabContent(record);
    renderProfileTreatmentHistoryTab(record);
}
 ...
 * @param {object} record - The full sheep record object.
 */
function renderProfileTimelineTabContent(record) {
    const container = document.getElementById('profileTimelineContainer');
    if (!container) return;

    const timelineEvents = [];
 
    // 1. Gather all weight entries
    const weightEntries = gatherAllWeightData(record);
    weightEntries.forEach(point => {
        timelineEvents.push({
            date: point.date,
            type: 'weight',
            data: point,
    ...
    // 2. Gather all treatment entries
    if (record.treatments) {
        Object.entries(record.treatments).forEach(([id, treatment]) => {
            timelineEvents.push({
                date: new Date(treatment.treatmentDate + 'T00:00:00'),
                type: 'treatment',
                data: treatment,
                id: id,
                recordId: record.id
    ...
    if (timelineEvents.length === 0) {
        container.innerHTML = '<tr><td colspan="4" class="text-center p-4 text-muted">No timeline events found.</td></tr>';
        return;
    }

    const rowsHtml = timelineEvents.map(event => {
        const formattedDate = formatDate(event.date.toISOString().split('T')[0]);
        let iconHtml = '';
        let detailsHtml = '';
        let actionButtons = '';

        if (event.type === 'weight') {
            iconHtml = '<i class="fas fa-weight-scale fa-2x text-info"></i>';
            detailsHtml = `
                <h6 class="mb-1">Weight Logged</h6>
                <p class="mb-0"><strong>${event.data.weight.toFixed(1)} kg</strong></p>
            `;
            actionButtons = `<button class="btn btn-sm btn-outline-primary js-edit-weight" data-record-id="${event.recordId}" data-entry-id="${event.id}" data-source="${event.data.source}" title="Edit Weight"><i class="fas fa-edit"></i></button>`;
        } else if (event.type === 'treatment') {
            const treatmentIcon = getTreatmentIcon(event.data.treatmentType);
            const treatmentBadgeClass = getTreatmentBadgeClass(event.data.treatmentType);
            iconHtml = `<i class="fas ${treatmentIcon} fa-2x text-primary"></i>`;
            detailsHtml = `
                <h6 class="mb-1">
                    ${event.data.medication || 'Treatment'}
                    <span class="badge ${treatmentBadgeClass} ms-2">${event.data.treatmentType}</span>
                </h6>
                <p class="mb-0 small">
                    <strong>Dosage:</strong> ${event.data.dosage || 'N/A'}
                    ${event.data.cost ? `| <strong>Cost:</strong> ${formatCurrency(event.data.cost)}` : ''}
                </p>
                ${event.data.symptoms ? `<p class="mb-0 small text-danger-emphasis"><em>Symptoms: ${escapeHTML(event.data.symptoms)}</em></p>` : ''}
                ${event.data.treatmentNotes ? `<p class="mb-0 small text-muted"><em>Notes: ${escapeHTML(event.data.treatmentNotes)}</em></p>` : ''}
            `;
            actionButtons = `<button class="btn btn-sm btn-outline-primary js-edit-treatment" data-record-id="${event.recordId}" data-entry-id="${event.id}" title="Edit Treatment"><i class="fas fa-edit"></i></button>`;
        }

        return `
            <tr>
                <td style="width: 100px;" class="text-center">
                    <div class="fw-bold">${formattedDate.split('/')[0]}/${formattedDate.split('/')[1]}</div>
                    <div class="small text-muted">${formattedDate.split('/')[2]}</div>
                </td>
                <td style="width: 60px;" class="text-center">${iconHtml}</td>
                <td>${detailsHtml}</td>
                <td style="width: 80px;" class="text-end">${actionButtons}</td>
            </tr>
        `;
    }).join('');
...
/**
 * Renders the dedicated Treatment History table for the profile page.
 * @param {object} record - The full sheep record object.
 */
function renderProfileTreatmentHistoryTab(record) {
    const container = document.getElementById('profileTreatmentHistoryContainer');
    if (!container) return;

    if (!record.treatments) {
        container.innerHTML = '<div class="p-4 text-center text-muted">No treatment history recorded.</div>';
        return;
    }

    const sortedEntries = Object.entries(record.treatments)
        .sort((a, b) => new Date(b[1].treatmentDate) - new Date(a[1].treatmentDate));
    ...
    const tableHtml = `
        <table class="table table-striped table-hover align-middle">
            ...
            <tbody>
                ${sortedEntries.map(([entryId, entry]) => {
                    const followUpDisplay = getFollowUpDateDisplay(entry.followUpDate);
                    const treatmentBadgeClass = getTreatmentBadgeClass(entry.treatmentType);
                    const treatmentIcon = getTreatmentIcon(entry.treatmentType);
...
/**
 * Initializes the UI elements and data for the Feed/Expenditure section.
 */
function initializeFeedSection() {
    const feedDateEl = document.getElementById('feedPurchaseDate');
    if (feedDateEl) {
        feedDateEl.valueAsDate = new Date();
    }
    populateExpenditureFilters();

    // Set default filter to current month and year for the first load
    const today = new Date();
    const yearFilter = document.getElementById('feedYearFilter');
    if (Array.from(yearFilter.options).some(opt => opt.value == today.getFullYear())) {
        yearFilter.value = today.getFullYear();
    }
    document.getElementById('feedMonthFilter').value = today.getMonth() + 1;
...
function initializeUI() {
    ...
    runSafely('Set Default Date', () => {
        ...
        const feedDateEl = document.getElementById('feedPurchaseDate');
        if (feedDateEl) feedDateEl.valueAsDate = new Date();
        updateElement('welcomeMessage', `Welcome, ${auth.currentUser?.displayName || 'User'}!`);
    });
...
function addEventListeners() {
    ...
    mainApp.addEventListener('click', (e) => {
        ...
        if (action = getAction('th.sortable')) {
            const tableKey = action.button.closest('table[data-table-key]')?.dataset.tableKey;
            const column = action.sort;
            if (tableKey && column && tableSorts[tableKey]) {
                if (tableSorts[tableKey].column === column) {
                    tableSorts[tableKey].direction = tableSorts[tableKey].direction === 'asc' ? 'desc' : 'asc';
                } else {
                    tableSorts[tableKey].column = column;
                    tableSorts[tableKey].direction = 'asc';
                }
                renderAllRecordTables();
            }
        }
        // Dashboard card links
        else if (action = getAction('.dashboard-card-link')) {
            if (action.section) {
        ...
        else if (action = getAction('.js-edit-treatment')) {
            const { recordId, entryId } = action;
            const record = [...allRecords, ...soldRecords, ...archivedRecords].find(r => r.id === recordId);
            if (record) {
        ...
        else if (target.closest('#sidebarToggleBtn')) {
    ...
    mainApp.addEventListener('change', e => {
        ...
        } else if (e.target.matches('#treatmentFeedType')) {
            // When feed type changes, re-validate and re-calculate cost
            validateFeedQuantity();
            const treatmentType = document.getElementById('treatmentType').value;
            if (treatmentType === 'Feed' || treatmentType === 'Medical') {
                toggleTreatmentFormForFeed(treatmentType); // Re-evaluate UI based on selection
                calculateFeedCost();
            }
        } else if (e.target.matches('#dosage')) {
            if (document.getElementById('treatmentType').value === 'Feed') calculateFeedCost();
        } else if (e.target.matches('#batchTreatmentType')) {
            toggleBatchDueDateField(e.target.value);
        }
    });

    // --- Expenditure Log Filter Listeners ---
    addSafeEventListener('feedMonthFilter', 'change', renderExpenditureLog);
    addSafeEventListener('feedYearFilter', 'change', renderExpenditureLog);
    addSafeEventListener('clearFeedFiltersBtn', 'click', () => {
        document.getElementById('feedMonthFilter').value = 'all';
        document.getElementById('feedYearFilter').value = 'all';
        renderExpenditureLog();
    });
    ...
    // Real-time validation for feed quantity input
    addSafeEventListener('dosage', 'input', () => {
        // Only validate if the current treatment type is 'Feed'
        if (document.getElementById('treatmentType').value === 'Feed') validateFeedQuantity();
    });
}
